...
import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QTabWidget, QLabel, QWidget, QTableWidget, \
...
class XRF_interface(QMainWindow):
    ...
    def __init__(self):
        ...
        self.table.setSelectionBehavior(
            QAbstractItemView.SelectRows)  # ao clicar em um item a linha toda será selecionada
    ...
    def select_data(self):
        ...
        a = np.zeros([51, 51, 4105], dtype=np.float)
        dado=0
        ...
        for i in range(51):
            ...
            for j in range(51):
                a[i][j] = dados[j][0]+dados[j][1]+dados[j][2]+dados[j][3]
                dado=a[i,j]+dado


        self.Data = hdf5File(self.filename, '')

        self.Data.xrf_data=a
        self.Data.xrf_stackdata=dado
    ...
    def Energy_line(self):
        ...
        if Bclick == [None]:
            ...
            if len(Bclick) > 1:  # Caso um  nivel de energia for escolhido
                ...
                if (
                        Bclick in self.Elements) == False:  # Caso o elemento selecionado ainda nao foi escolhido pelo usuario
            ...
            try:
                ...
                def hoverEvent(event):  # funcao para obter o pixel do mouse
                    if event.isExit():
                        ...
                        image.setTitle('pixel: (%d, %d) &nbsp;&nbsp;&nbsp; Value[R,G,B]: (%d,%d,%d)' % (
                        i, j, val_red, val_green, val_blue))
    ...
    def img_show_window(self):
        ...
        try:
            ...
            layout.setStretch(0, 0.5)
            layout.setStretch(1, 6.5)
            ...
            def hoverEvent(event):  # funcao para obter o pixel do mouse
                if event.isExit():
                    ...
                    p1.plot(self.Data.Energy, self.Data.xrf_data[i, j], pen='b')
                    image.setTitle('pixel: (%d, %d) &nbsp;&nbsp;Value: %d' % (i, j, val))
